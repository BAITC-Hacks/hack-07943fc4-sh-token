...
2. Python-пайплайн проверяет данные, считает признаки, роли, кластеры и
   ...
   `public/data/`.
3. В браузере аналитик нажимает «Проанализировать сеть» и видит сохранённый
...
Кнопка в браузере загружает ранее рассчитанный `graph.json` и показывает этапы
работы. Новый пересчёт parquet запускается через CLI, а не браузерную кнопку.
...
«Проанализировать сеть», выберите первый узел топ-листа, затем найдите
